...
function getIcon(name: string) {
  switch (name?.toLowerCase()) {
    ...
    case 'heart':
    default:
      return <Heart className="w-8 h-8 text-amber-500" />
  }
}

export default async function VivencieOSagradoPage() {
  const data = await getPaginaVivencie()

  const tituloBanner = data?.tituloBanner || 'Vivencie o Sagrado'
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-16 space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-6 relative h-80 rounded-md overflow-hidden shadow-lg border-2 border-slate-200">
            <Image
              src={imageUrl}
              alt="Alunos convivendo no Colégio Sagrado Coração de Jesus"
              fill
